...
KGPL::Scene::Scene(const ShadersPath Shaders2D, const ShadersPath Shaders3D, bool two, bool three) {
	if (two) 
		ShaderProgram2D = RSU::CreateProgram(
			RSU::CompileVertexShaderFromFile(Shaders2D.VertexShader),
			RSU::CompileFragmentShaderFromFile(Shaders2D.FragmentShader),
			RSU::CompileGeometryShaderFromFile(Shaders2D.GeometryShader)
		); else ShaderProgram2D = NULL;

	if (three)
		ShaderProgram3D = RSU::CreateProgram(
			RSU::CompileVertexShaderFromFile(Shaders3D.VertexShader),
			RSU::CompileFragmentShaderFromFile(Shaders3D.FragmentShader),
			RSU::CompileGeometryShaderFromFile(Shaders3D.GeometryShader)
